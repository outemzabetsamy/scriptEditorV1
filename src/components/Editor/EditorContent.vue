...
export default {
  ...
  created() {
    this.document.onKeyDown= ()=>{
      console.log("keydooooooooooooooooooooooooooooooooooooown")
    }
  ...
  methods: {
    AddScene(scene){
      ...
      if(scene ==='scene'){
        ...
        ,insert:'h6',content:`${this.selectedBlock?this.selectedBlock+1:this.document.length+1} `,attrs:{align:'left',bold:true}}],attrs:{align:'left',bold:true}})
      }
      ...
      /*this.scenes.push( {
        idScene:`${this.scenes.length-1}-${$char.random(5)}`,
        repere:'EXT',
        momentDeLaJournée:'MOMENT',
        lieu:'Lieu',
        action:'//ACTION',
        dialogue:[
            {idReplique:1,personnageReplique:'ACTEUR1',parentheseReplique:'Sentiment',texteReplique:'Dialogue1',ordre:1},
            {idReplique:2,personnageReplique:'ACTEUR2',parentheseReplique:'Sentiment',texteReplique:'Dialogue2',ordre:2},
        ]
  ...
  AddSceneInline(scene){
        ...
        if(nodes.length >= 1) {
          ...
          nodes.forEach(node => {
            ...
            if(typeof node === 'object' && node !== null) {
              ...
              if(node.insert === 'span') {
                console.log("span")
                contentNode.push(h(node.insert, { id: node.id, class: [this.toggleAttributes(node.attrs),this.typeAttributes(node.attrs)], onClick: this.storeTextNode, index:`${i}` }, this.subNodes(node.content)))
              
              } else if(node.insert === 'div') {
                console.log("a")
                contentNode.push(h(node.insert, { id: node.id, class: [this.toggleAttributes(node.attrs),this.typeAttributes(node.attrs)], index:`${i}`}, this.subNodes(node.content)))
    ...
    enterNewBlock() {
     ...
     if(event.key === 'Enter') {
      ...
      console.log("event key uuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuuup")
